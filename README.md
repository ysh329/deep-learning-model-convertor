...
### [AlexPasqua/keras-caffe_converter_optimizer](https://github.com/AlexPasqua/keras-caffe_converter_optimizer)

Convert Keras models to Caffe nets. It takes a Keras model and creates the Caffe's `prototxt` and `caffemodel`, it's also possible to use your own `prototxt` in case it's present.<br>
It works also in the opposite direction (from Caffe to Keras).
...
It takes the `prototxt` and the `caffemodel` and convert a Caffe net into a Keras model.<br>
It works also in the opposite direction (from Keras to Caffe).
